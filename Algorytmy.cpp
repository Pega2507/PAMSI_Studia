...
void merge(T *tab, T *tmp, int left, int midd, int right) //sortowanie
{
	for (int i = left; i <= right; i++) 
		tmp[i] = tab[i];			//przypisanie tabicy do tmp
...
template<typename T>
void mergesort(T *tab, T *tmp, int left, int right)  //rozdzielanie
{
	if (left < right)
	{
...
{
    int pivot = tab[(left + right) / 2], i = left, j = right; //wybrana os zawsze bedzie srodkowa
